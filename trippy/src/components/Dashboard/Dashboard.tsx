...
const Dashboard = () => {
  const { setDashboardData, dashboardData, activeTab, dashboardNeedsRefresh, setDashboardNeedsRefresh } = useDataContext();
...
}, [activeTab, dashboardNeedsRefresh]);
          ...
          <div className="w-full overflow-x-auto">
            <div className="flex items-end justify-between h-44 md:h-64 min-w-full gap-1 md:gap-1.5">
              {monthlyTotals.map((item, idx) => {
                ...
                const height =
                  maxMonthlyAmount > 0 ? (revenue / maxMonthlyAmount) * 100 : 0;
